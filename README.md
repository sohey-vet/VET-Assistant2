...
- **過去数か月の重複チェック**: 3-24か月前の投稿との内容重複を厳重に監視
- **多層的重複検出**: 完全一致・類似度判定・キーワード抽出による高精度チェック
- **自動再生成機能**: 重複検出時は最大5回まで自動再生成
- **SQLite履歴管理**: 投稿履歴を永続的に保存・管理
...
### アプリケーション起動
- **強化版（推奨）**: `run_enhanced.bat` をダブルクリック
